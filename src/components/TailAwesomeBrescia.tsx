  ...
  return (
    <div className="min-h-screen bg-[#f7f9fc] text-slate-950">
      ...
      <main>
        ...
        <section id="servizi" className="py-20 sm:py-28"><Container><div className="max-w-2xl"><p className="text-xs font-black uppercase tracking-[0.24em] text-[#e85d32]">Interventi</p><h2 className="mt-4 text-4xl font-black tracking-[-0.05em] text-[#0f3d6e] sm:text-6xl">Servizi pensati per ogni fermo veicolo.</h2><p className="mt-6 text-lg leading-8 text-slate-600">Ogni mezzo richiede un recupero diverso. Per questo valutiamo condizioni, accessi e destinazione prima di organizzare il trasporto.</p></div><div className="mt-12 grid gap-4 md:grid-cols-2 lg:grid-cols-3">{services.map((service,index)=><Link key={service.slug} href={`/servizi/${service.slug}/`} className="group border border-slate-200 bg-white p-6 shadow-sm transition hover:-translate-y-1 hover:border-[#e85d32] hover:shadow-xl"><div className="flex items-center justify-between"><span className="text-xs font-black text-[#e85d32]">0{index+1}</span><span className="text-slate-400">â†—</span></div><h3 className="mt-12 text-xl font-black text-[#0f3d6e]">{service.title}</h3><p className="mt-3 text-sm leading-6 text-slate-600">{service.text}</p></Link>)}</div></Container></section>

        <section id="zone" className="bg-[#0f3d6e] py-20 text-white sm:py-28"><Container className="grid gap-10 lg:grid-cols-[0.7fr_1.3fr]"><div><p className="text-xs font-black uppercase tracking-[0.24em] text-orange-300">Reggio Emilia e dintorni</p><h2 className="mt-4 text-4xl font-black tracking-[-0.04em] sm:text-6xl">Dove possiamo aiutarti.</h2><p className="mt-6 text-lg leading-8 text-blue-100">Dalla cittÃ  ai comuni vicini, organizziamo soccorso, traino e trasporto per auto, moto e furgoni.</p></div><div className="grid gap-x-8 sm:grid-cols-2">{zones.map(zone=><Link key={zone.slug} href={`/zone/${zone.slug}/`} className="flex justify-between border-b border-white/20 py-4 font-bold text-white transition hover:border-orange-300">{zone.name}<span>â†—</span></Link>)}</div></Container></section>

        <section id="faq" className="bg-white py-20 sm:py-28"><Container className="max-w-4xl"><p className="text-xs font-black uppercase tracking-[0.24em] text-[#e85d32]">FAQ</p><h2 className="mt-4 text-4xl font-black tracking-[-0.05em] text-[#0f3d6e] sm:text-6xl">Le risposte prima della chiamata.</h2><div className="mt-10 divide-y divide-slate-200 border-y border-slate-200">{faqs.map((faq)=><details key={faq.question} className="group py-6"><summary className="flex cursor-pointer list-none justify-between gap-5 text-lg font-black text-slate-950"><span>{faq.question}</span><span className="text-2xl text-[#e85d32] transition group-open:rotate-45">+</span></summary><p className="mt-4 max-w-2xl text-base leading-7 text-slate-600">{faq.answer}</p></details>)}</div></Container></section>
